                            ...
                            char **cookies, int cookies_count, char** tokens)
{
    ...
    // Step 4 (optional): add Authorization JWT header
    if (tokens != NULL) {
        memset(line, 0, LINELEN);
        sprintf(line, "Authorization: %s", tokens[0]);
                            ...
                            int body_data_fields_count, char **cookies, int cookies_count, char **tokens)
{
    ...
    // body_data_size = strlen(body_data);
    ...
    // Step 5 (optional): add Authorization JWT header
    if (tokens != NULL) {
        memset(line, 0, LINELEN);
        sprintf(line, "Authorization: %s", tokens[0]);
